...
def Discriminator(image_tensor,encoded_sentences_tensor,reuse_flag):
    with tf.variable_scope(tf.get_variable_scope(),reuse=reuse_flag):
        ...
        d_a1=tf.nn.conv2d(d_a0,d_w1,strides=[1,1,1,1],padding='SAME')+d_b1
        ...
        d_w7=tf.get_variable('d_w7',shape=[2*2*2048,1],initializer=tf.contrib.layers.xavier_initializer())
        d_b7=tf.get_variable('d_b7',shape=[1],initializer=tf.contrib.layers.xavier_initializer())
        d_a7=tf.matmul(d_a6,d_w7)+d_b7
...
d_trainer=tf.train.AdamOptimizer(0.9e-3,beta1=0.1).minimize(W_d,var_list=d_vars)
...
saver.restore(sess,'./saved_models-soumya/c2i-77200')
